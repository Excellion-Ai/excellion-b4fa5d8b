...
export function BuilderShell() {
  ...
  const loadProject = async (id: string) => {
    ...
    // Load saved data from spec
    const spec = data.spec as { html?: string; messages?: Message[] } | null;
    if (spec) {
      if (spec.html) {
  ...
  // Auto-generate if we have an initial idea from the hub (and no projectId)
  useEffect(() => {
  ...
  const saveProject = async (html: string | null, allMessages: Message[], ideaText: string) => {
  ...
  const handleGenerate = async (inputIdea?: string) => {
    ...
    try {
      ...
      // Save/update project to database with full chat history
      const firstUserMessage = allMessages.find(m => m.role === 'user');
      await saveProject(html, allMessages, firstUserMessage?.content || ideaToUse);
    } catch (error) {
